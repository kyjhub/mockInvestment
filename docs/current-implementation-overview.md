...
1. 현재 처리할 주문
2. **이 매칭이 건드릴 수 있는 계좌 전부 — id 오름차순으로 한 번에**
3. 잠근 계좌와 종목의 보유 row
4. 내부 반대 주문 후보

이미 취소 또는 체결 완료된 주문은 row lock 획득 후 상태를 다시 확인하고 바로 반환한다.

#### 계좌 락을 한 번에, id 순으로 잡는 이유

예전에는 진입 시점에 자기 계좌를 잡고 내부 체결 때 상대 계좌를 추가로 잡았다. 그러면 종목 A의 매칭이 계좌1→계좌2 순으로, 종목 B의 매칭이 계좌2→계좌1 순으로 잡을 수 있다. **심볼 락은 종목 단위라 두 매칭이 동시에 돈다** — 스트림 컨슈머(100ms)와 dirty drainer(150ms)가 서로 다른 스레드다. 데드락이 실제로 성립했고, PostgreSQL이 감지해 한쪽을 abort할 때까지 기다려야 했다.

지금은 `matchOrder()`가 시작 시점에 상대 후보 계좌를 **락 없이** 읽고, 자기 계좌까지 합쳐 **id 오름차순으로** 하나씩 잠근다. 모든 트랜잭션의 획득 순서가 같으므로 순환이 만들어지지 않는다.

읽기를 락 없이 하는 이유는, 락을 걸면 순서를 정하려는 일을 순서 없이 해버리기 때문이다.
...
**상한을 넘는 상대가 필요한 큰 주문도 이 스윕 안에서 다 채워진다.** `matchSymbol()`이 그 종목의 미체결 주문을 전부 순회하므로, 상한에 밀린 상대들이 각자 자기 차례에 그 주문을 상대로 체결하기 때문이다. 상한은 "한 트랜잭션이 잠그는 범위"를 정할 뿐 체결 범위를 정하지 않는다.

후보 조회는 **미리 잠긴 계좌로 한정**한다. 잠기지 않은 계좌의 주문을 후보로 돌려주면, 매칭이 그 주문 row를 하나씩 잠가 가며 제외 목록에 넣는다. 체결하지도 못할 주문을 잠그는 셈이라 낭비이자 불필요한 락 경합이다.
...
| `SelfTradeAndConstraintIntegrationTest` | 7 | 통합 | 자전거래 차단, 정상 내부 체결 유지, 교차 종목 동시 매칭 데드락 부재, 후보 상한이 가격 우선순위를 뒤집지 않음, 상한을 넘는 큰 주문의 스윕 내 완전 체결, 음수 잔고·보유 DB 거부 |
...
2026-09-12 기준 `./gradlew test --rerun-tasks`는 **83건 전부 통과**한다. Testcontainers를 쓰므로 실행 환경에 Docker가 필요하다.
