...
전역 활성 종목을 (우선순위, symbol) 순으로 정렬한 뒤, **종목 이름 해시로 소유 슬롯을 정한다.**

> **갱신 (07번 계획에서 수정됨)** — 최초에는 인덱스 홀짝으로 나눴으나, 그 방식은 모든 인스턴스의 목록이
> 완전히 같아야만 성립한다. 구독·주문 추가 시점 차이나 Redis 장애로 목록이 갈리면 같은 위치가 서로 다른
> 종목을 가리켜 중복 구독과 누락이 함께 발생한다. 아래 서술은 그 이유로 대체됐다.
...
해시를 쓰면 목록이 갈려도 같은 종목은 언제나 같은 슬롯에 속하므로 **중복 구독이 구조적으로 불가능**하다.
`String.hashCode()`는 명세로 고정된 값이라 JVM이나 인스턴스가 달라도 동일하다.

대신 한쪽에 몰려 연결당 한도(100)를 넘을 수 있으므로, 넘으면 우선순위 상위부터 자르고 나머지는 REST 폴링이 가져간다.
실제 티커로 측정한 결과 손실은 **활성 종목이 정확히 정원(200)일 때만** 발생하고 평균 5종목, 최대 18종목이다
(100종목이나 300종목 이상에서는 손실이 없다). 그 정도는 남는 REST 예산으로 충분히 흡수된다.
