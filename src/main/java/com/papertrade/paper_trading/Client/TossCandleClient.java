...
public class TossCandleClient {
    ...
    public CandleResponse getLatestDailyCandle(String symbol) {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(latestDailyCandleUri(symbol))
        ...
        try {
            ...
            throw new IllegalStateException("Failed to call Toss candles API", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Toss candles API call was interrupted", e);
        }
    }
    ...
    private CandleResponse handleResponse(HttpResponse<String> response) {
        if (response.statusCode() >= 200 && response.statusCode() < 300) {
            ...
            try {
                return objectMapper.readValue(response.body(), CandleResponse.class);
